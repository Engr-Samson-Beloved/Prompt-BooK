...
const FeatureCard: React.FC<{ service: typeof services[0], idx: number }> = ({ service, idx }) => {
  ...
  return (
    <motion.div
      ...
      className="glass-card p-10 flex flex-col relative overflow-hidden cursor-default group border border-neutral-100 hover:border-brand-accent/20 transition-colors"
      style={{ transformStyle: 'preserve-3d' }}
    >
      {/* Spotlight Effect */}
      <div 
        ref={spotlightRef}
        className="absolute w-[200px] h-[200px] bg-brand-accent/5 blur-3xl rounded-full pointer-events-none opacity-0"
      />
      ...
      <div className="mt-4 relative z-10">
        ...
        <p className="text-neutral-500 leading-relaxed font-sans text-sm md:text-base pr-4">
          {service.desc}
        </p>
      </div>
    ...
    <section className="py-24 max-w-7xl mx-auto px-6 overflow-hidden">
      <div className="flex flex-col items-center mb-20">
        <motion.div
          ...
          className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-neutral-50 border border-neutral-100 mb-6 font-mono text-[10px] text-neutral-400 uppercase tracking-widest"
        >
        ...
        <motion.h2 
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          className="text-5xl md:text-7xl font-display mb-4 text-center tracking-tighter"
        >
          Core <span className="text-brand-accent">Capabilities</span>
        </motion.h2>
        <p className="text-neutral-500 text-center max-w-lg font-sans">
          Autonomous modules designed to bridge the gap between imagination and production.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <motion.div 
          ...
          whileInView={{ opacity: 1, scale: 1 }}
          className="bg-brand-black rounded-[2.5rem] p-12 flex flex-col justify-between text-white relative overflow-hidden group shadow-2xl"
        >
          <div className="relative z-10">
            <h3 className="text-4xl font-display mb-6 max-w-[200px] leading-tight">Get started with promptBook</h3>
            <button 
              onClick={onOpenConnect}
              className="px-10 py-4 bg-white text-brand-black rounded-full font-mono font-bold uppercase tracking-widest text-xs hover:bg-brand-accent hover:text-white transition-all shadow-xl"
            >
              INITIALIZE_SYNC
            </button>
          </div>
        ...
        {services.map((service, idx) => (
          <FeatureCard key={service.title} service={service} idx={idx} />
        ))}
      </div>
