...
const IdeaExpanderFlow: React.FC = () => {
  ...
  // PERSISTENCE: Save/Load Neural Vault
  useEffect(() => {
  ...
  useEffect(() => {
    localStorage.setItem('startup_architect_vault', JSON.stringify(savedBlueprints));
  }, [savedBlueprints]);

  const handleSaveBlueprint = () => {
    if (!result) return;
    const exists = savedBlueprints.some(b => b.product_name === result.product_name);
    if (!exists) {
      setSavedBlueprints([result, ...savedBlueprints]);
    }
  };

  const handleDeleteBlueprint = (name: string) => {
    setSavedBlueprints(savedBlueprints.filter(b => b.product_name !== name));
  };

  const handleExportMarkdown = (blueprint: NeuralBlueprint) => {
    const content = `
# ${blueprint.product_name} - Startup Blueprint
Generated by Prompt-BooK Neural Core
...
- **Pricing**: ${blueprint.monetization?.pricing_tiers.join(', ')}
